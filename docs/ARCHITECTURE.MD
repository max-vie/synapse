...
Production modules live in `src/synapse/`. Repository operations are separate: `scripts.lab` owns the local Compose lifecycle, `scripts.proof` owns evidence scenarios, and benchmark/capture tooling stays optional.

## Code map

- `metadata.py`: pure note identity, path safety, hashing, and chunking policy.
- `ingest.py`: note limits, formatting, Qdrant replacement, Wiki.js publishing, rollback, and deletion ordering.
- `rag.py`: typed `Question`, `RetrievedChunk`, `Source`, and `AnswerResult` values plus the retriever, generator, and validator interfaces.
- `ask.py`: grounding policy and the Ollama/Qdrant adapters behind the typed RAG seam.
- `runtime.py`: the transport adapter and the three operations exposed to the HTTP layer.
- `service.py`: authentication, request limits, readiness, concurrency, and compatibility route registration.
- `scripts/evaluate.py`: deterministic in-memory evaluation proof; it does not call a model or network service.

When changing retrieval behavior, start at `rag.py` and `ask.py`. When changing note lifecycle behavior, start at `metadata.py` and `ingest.py`. Keep the HTTP layer thin so those policies remain testable without Docker.
...
The source-grounded RAG interface is `Question -> AnswerResult`. Qdrant retrieval, Ollama generation, and answer validation are adapters behind `Retriever`, `Generator`, and `AnswerValidator` ports. The HTTP layer injects a frozen `Settings` value and `SynapseRuntime` at the application seam; the module-level app remains only as a compatibility adapter for local callers.
...
For a signed-in cloud run, `OLLAMA_CLOUD_MODE=true` adds `docker-compose.cloud-e2e.yml`. That overlay mounts the operator's Ollama key pair read-only into the isolated relay. Cloud-capable format and answer models (for example `gpt-oss:20b-cloud`) are offloaded by Ollama; `nomic-embed-text` remains a local embedding model because Ollama Cloud does not currently expose embedding models. The application still calls the same internal Ollama API, so cloud mode exercises the production-shaped request and retrieval path without reusing the host daemon.
...
This prevents duplicate-index buildup when the same note is edited and reprocessed, and avoids a visible Wiki.js page getting ahead of a failed Qdrant update. Publication identity is derived from the normalized vault path, so a title change updates the same Wiki.js path. A delete request removes Qdrant chunks before attempting Wiki.js page deletion; this keeps stale content out of grounded answers if the publisher is unavailable. It is still a local-lab consistency model, not a distributed transaction.
...
- `POST /webhook/synapse/note` with `delete: true`: removes the stable note identity from Qdrant and Wiki.js.
...
- `SYNAPSE_MAX_REQUEST_BYTES`: complete JSON request-body limit, default `1048576` bytes; requests above it receive HTTP 413 before JSON parsing.
...
- `SYNAPSE_HTTP_TIMEOUT_SECONDS`: timeout for external service requests, default `180` seconds for slower local models.
...
The live answer gate is a source-grounding boundary, not a general fact checker. Before accepting an answer, it rejects empty or refused answers, scans every citation for a valid source number, requires at least one valid citation at the end of the answer, and requires every cited source to expose a locator such as `source_path`, `wiki_path`, `source_url`, or `note_id`. Each source also includes `quoted_support`, a selected excerpt from the retrieved chunk that makes human review possible.

A valid response shape is:

```json
{
  "question": "What does Ollama provide in the knowledge system?",
  "answer": "Ollama provides local formatting, embedding, and answer generation. [1]",
  ...
  "sources": [
    {
      ...
      "score": 0.91,
      "chunk_index": 0,
      "quoted_support": "Ollama provides local formatting, embedding, and answer generation."
    }
  ],
  "retrieval": {
    "accepted": 1,
    ...
    "query_terms": ["ollama", "provide", "knowledge", "system"],
    "anchor_terms": ["ollama", "provide", "knowledge", "system"],
    "matched_terms": ["ollama", "provide", "knowledge", "system"],
    "term_coverage": 1.0,
    "required_matches": 3,
    "anchors_satisfied": true,
    "answer_validation": "quote_overlap"
  }
}
```

Scores, identifiers, paths, and retrieval diagnostics vary with the indexed note and environment; the example shows the complete public result shape for one accepted source.

The default `quote_overlap` mode checks citation mechanics and requires a minimum trigram overlap between the answer and the quoted support. `extractive` requires the answer content to appear verbatim in quoted support. `structural` remains available only as an explicitly diagnostic mode and checks citation mechanics without content grounding. Overlap demonstrates textual support, not that the source itself is factually correct or that the model understood it; the deterministic evaluation and real-stack proof report contract behavior separately from model quality.
...
- Webhook auth fails closed: live FastAPI webhooks require the file-backed `SYNAPSE_WEBHOOK_AUTH_TOKEN_FILE` (with a temporary legacy inline-variable fallback) and either the `X-Synapse-Token` header or a bearer-token `Authorization` header. The only no-token bypass is explicit `SYNAPSE_AUTH_DISABLED=true` for no-network demo runs.
